...
import { FiArrowLeft, FiStar, FiCheck, FiClock, FiDollarSign, FiMapPin } from "react-icons/fi";
...
const companyData: Record<string, Service> = {
  "preservacao-reflorestamento": {
    ...
    since: 2015,
    ods: [13, 15],
    fullDescription: "A Empresa Verde é líder em projetos de reflorestamento, utilizando técnicas avançadas de recuperação de áreas degradadas. Nossos métodos incluem análise de solo personalizada, seleção de espécies nativas e monitoramento pós-plantio para garantir o sucesso do reflorestamento.",
    benefits: [
    ...
    pricing: [
      { option: "Pacote Básico", price: "R$ 15.000", details: "Até 5 hectares" },
      { option: "Pacote Intermediário", price: "R$ 28.000", details: "5 a 15 hectares" },
      { option: "Pacote Completo", price: "R$ 50.000", details: "Acima de 15 hectares" }
    ]
  },
  "limpeza-rios": {
    ...
    description: "Tecnologia avançada para remoção de resíduos e despoluição de cursos d'água.",
    reviews: 89,
    since: 2018,
    ods: [6, 14],
    fullDescription: "A Aqua Clean Solutions desenvolveu metodologias inovadoras para limpeza de rios urbanos e naturais, combinando tecnologia de ponta com processos ecológicos para remoção eficiente de poluentes sem danificar o ecossistema aquático.",
    benefits: [
    ...
    pricing: [
      { option: "Limpeza Básica", price: "R$ 12.000", details: "Até 3km de extensão" },
      { option: "Limpeza Completa", price: "R$ 25.000", details: "3 a 8km de extensão" },
      { option: "Projeto Personalizado", price: "Sob consulta", details: "Acima de 8km" }
    ]
  },
  "limpeza-praias": {
    company: "Oceano Vivo",
    ...
    since: 2017,
    ods: [6, 14, 12],
    fullDescription: "A Oceano Vivo realiza operações de limpeza em praias e áreas costeiras, com tecnologia para remoção de microplásticos e resíduos sólidos. Nossas equipes trabalham em parceria com comunidades locais e órgãos ambientais.",
    benefits: [
    ...
    pricing: [
      { option: "Ação Mensal", price: "R$ 18.000", details: "Manutenção regular" },
      { option: "Ação Trimestral", price: "R$ 45.000", details: "Pacote econômico" },
      { option: "Limpeza Emergencial", price: "Sob consulta", details: "Para desastres ambientais" }
    ]
  },
  "educacao-treinamento": {
    ...
    since: 2019,
    ods: [4, 5, 10],
    fullDescription: "A Eco Educa Brasil desenvolve programas educacionais voltados para sustentabilidade ambiental, capacitando comunidades e instituições de ensino com metodologias práticas e inclusivas.",
    benefits: [
    ...
    pricing: [
      { option: "Workshop Básico", price: "R$ 5.000", details: "4 horas de capacitação" },
      { option: "Curso Completo", price: "R$ 15.000", details: "20 horas de capacitação" },
      { option: "Programa Anual", price: "R$ 50.000", details: "Capacitação contínua" }
    ]
  },
  "energia-sustentavel": {
    ...
    since: 2016,
    ods: [7, 8, 11],
    fullDescription: "A Solar Power BR democratiza o acesso à energia limpa através de soluções personalizadas e financiamento acessível, levando tecnologia solar para comunidades carentes e pequenos negócios.",
    benefits: [
    ...
    pricing: [
      ...
      { option: "Sistema Residencial", price: "R$ 18.000", details: "2 a 5kWp" },
      { option: "Sistema Comercial", price: "Sob consulta", details: "Acima de 5kWp" }
    ]
  }
};

export default function ServiceDetails({ params }: { params: Promise<{ serviceId: string }> }) {
    const router = useRouter();
    const resolvedParams = React.use(params); // Desempacota a Promise
    const service = companyData[resolvedParams.serviceId as keyof typeof companyData];
    ...
    const renderProjectMetric = (project: Project) => {
        if (project.trees) return `${project.trees.toLocaleString()} árvores plantadas`;
        if (project.length) return `Extensão: ${project.length}`;
        if (project.students) return `${project.students.toLocaleString()} capacitados`;
        if (project.systems) return `${project.systems.toLocaleString()} sistemas instalados`;
  ...
  <main className="min-h-screen w-full flex items-center justify-center bg-gradient-to-b from-gray-900 to-black px-4 sm:px-6 lg:px-8 min-w-[320px]">
            ...
            >
                ...
                <div className=" rounded-2xl  mt-6">
                    ...
                    <Link
                        href="/Chat"
                        className="inline-block px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-medium transition-colors cursor-pointer"
                    >
                ...
                >
                    ...
                    <h3 className="text-xl font-bold mb-4 flex items-center">
                        <FiCheck className="text-green-400 mr-2" /> Benefícios do Serviço
                    </h3>
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {service.benefits.map((benefit, index) => (
                        <li key={index} className="flex items-start gap-2 text-zinc-300">
                            <FiCheck className="text-green-400 mt-1 flex-shrink-0" /> {benefit}
                        </li>
                        ))}
                    </ul>
                    </div>
                    ...
                    <div className="mb-8">
                    <h3 className="text-xl font-bold mb-4 flex items-center">
                        <FiMapPin className="text-blue-400 mr-2" /> Projetos Realizados
                    </h3>
